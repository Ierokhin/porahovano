...
import { ID, TAX, COINS, STAKING, STABLECOINS, addToPortfolio, isInPortfolio } from "@/lib/instruments";
...
const netPct = (rate) => (rate * (1 - TAX.alternative)).toFixed(1) + "%";
...
function AddBtn({ added, onAdd }) {
  return <button onClick={onAdd} style={{width:"100%",padding:"6px 8px",borderRadius:8,fontSize:11,fontWeight:700,cursor:added?"default":"pointer",border:`1.5px solid ${added?T.green:T.border}`,background:added?T.greenLt:T.white,color:added?T.green:T.gray}}>{added?"✓ Додано":"+ Капітал"}</button>;
}
...
function MetalsSection() {
  ...
  // Показуємо "✓ Додано" якщо вже в портфелі (додано тут чи в калькуляторі)
  useEffect(() => {
    if (!commodities.length) return;
    const initial = {};
    commodities.forEach(c => {
      if (isInPortfolio(ID.commodity(c.id))) initial[c.id] = true;
    });
    setAdded(initial);
  }, [commodities]);
  ...
  function handleAdd(item) {
    ...
    const icon = item.sector === "metals" ? "🥇" : "🛢";
    addToPortfolio({
      productId: ID.commodity(item.id), name: `${icon} ${item.ticker}`,
      sub: item.sub ?? `${item.name} · EUR · LSE`,
      rate: item.rate, cur: "eur", tax: TAX.alternative, risk: item.risk ?? "mid", gtee: "IBKR",
    });
    setAdded(p => ({ ...p, [item.id]: true }));
    setToast(item.ticker);
      ...
      {state === "ok" && filtered.map((item, i, arr) => {
        ...
        return (
          <div key={item.id} style={{ display:"grid", gridTemplateColumns:colGrid, padding:"12px 16px", alignItems:"center", border:`1px solid ${T.border}`, borderTop: i===0 ? `1.5px solid ${T.border}` : "none", borderRadius: i===0 ? "14px 14px 0 0" : i===arr.length-1 ? "0 0 14px 14px" : 0, background: i===0 ? T.greenLt : T.white }}>
            ...
            <div><AddBtn added={isDone} onAdd={() => handleAdd(item)}/></div>
          </div>
        );
      })}
...
export default function AlternatyvniPage() {
  ...
  // Показуємо "✓ Додано" для всього, що вже в портфелі (додано тут чи в калькуляторі)
  useEffect(() => {
    const initial = {};
    COINS.forEach(c => { if (isInPortfolio(ID.crypto(c.id))) initial[ID.crypto(c.id)] = true; });
    STAKING.forEach(s => { if (isInPortfolio(ID.staking(s.id))) initial[ID.staking(s.id)] = true; });
    STABLECOINS.forEach(s => { if (isInPortfolio(ID.stablecoin(s.id))) initial[ID.stablecoin(s.id)] = true; });
    realtyUa.forEach(r => { if (isInPortfolio(ID.realty(r.id))) initial[ID.realty(r.id)] = true; });
    setAdded(initial);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rates]);

  function markAdded(pid, name) {
    setAdded(p => ({ ...p, [pid]: true }));
    setToast(name);
    setTimeout(() => setToast(null), 4000);
  }

  function handleAddCoin(c, r) {
    const pid = ID.crypto(c.id);
    if (added[pid]) return;
    addToPortfolio({
      productId: pid, name: c.name, sub: `${c.sym} · крипто`,
      rate: r.returnPct, cur: "eur", tax: TAX.alternative, risk: "high", gtee: "Немає", color: c.color,
    });
    markAdded(pid, c.name);
  }

  function handleAddStaking(s) {
    const pid = ID.staking(s.id);
    if (added[pid]) return;
    addToPortfolio({
      productId: pid, name: s.name, sub: s.sub,
      rate: s.rate, cur: "eur", tax: TAX.alternative, risk: "mid", gtee: "Немає",
    });
    markAdded(pid, s.name);
  }

  function handleAddStablecoin(s) {
    const pid = ID.stablecoin(s.id);
    if (added[pid]) return;
    addToPortfolio({
      productId: pid, name: s.name, sub: s.sub,
      rate: s.rate, cur: "eur", tax: TAX.alternative, risk: s.risk, gtee: "Немає",
    });
    markAdded(pid, s.name);
  }

  function handleAddRealty(r) {
    const pid = ID.realty(r.id);
    if (added[pid]) return;
    addToPortfolio({
      productId: pid, name: r.name, sub: "UAH · готель під управлінням",
      rate: r.rate, cur: "uah", tax: TAX.alternative, risk: "mid", gtee: "Немає",
    });
    markAdded(pid, r.name);
  }
                ...
                {coinState === "ok" && COINS.map(c => {
                  ...
                  return (
                    <div key={c.id} style={{ ...colGrid("32px 1fr 110px 110px 110px"), borderTop:`1px solid ${T.border}` }}>
                      ...
                      <div><AddBtn added={!!added[ID.crypto(c.id)]} onAdd={() => handleAddCoin(c, r)}/></div>
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{ border:`1.5px solid ${T.border}`, borderRadius:14, overflow:"hidden" }}>
              ...
              {STAKING.map(s => (
                <div key={s.id} style={{ ...colGrid("32px 1fr 90px 90px 1fr 110px"), borderTop:`1px solid ${T.border}` }}>
                  ...
                  <div style={{fontSize:13,fontWeight:700,color:T.amber}}>~{netPct(s.rate)}</div>
                  <div style={{fontSize:12,color:T.gray}}>{s.platform}</div>
                  <div><AddBtn added={!!added[ID.staking(s.id)]} onAdd={() => handleAddStaking(s)}/></div>
                </div>
              ))}
            </div>
          )}
            ...
            <div style={{ border:`1.5px solid ${T.border}`, borderRadius:14, overflow:"hidden" }}>
              ...
              {STABLECOINS.map(s => (
                <div key={s.id} style={{ ...colGrid("1fr 90px 90px 90px 110px"), borderTop:`1px solid ${T.border}` }}>
                  <div><div style={{fontWeight:700,fontSize:13}}>{s.name}</div><div style={{fontSize:11,color:T.gray}}>{s.sub}</div></div>
                  <div style={{fontSize:15,fontWeight:700,color:T.green}}>{s.display}</div>
                  <div style={{fontSize:13,fontWeight:700,color:T.amber}}>~{netPct(s.rate)}</div>
                  <div><RiskBadge risk={s.risk}/></div>
                  <div><AddBtn added={!!added[ID.stablecoin(s.id)]} onAdd={() => handleAddStablecoin(s)}/></div>
                </div>
              ))}
            </div>
          )}
      ...
      {mainTab === "realty" && (
        <div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:20 }}>
            {realtyUa.map((r, i) => (
              <div key={r.id} style={{ border:`1.5px solid ${i===0?T.green:T.border}`, borderRadius:14, padding:18, background:i===0?T.greenLt:T.white }}>
                <a href={r.url} target="_blank" rel="nofollow noopener" style={{ textDecoration:"none", color:"inherit", display:"block" }}>
                ...
                <div style={{display:"flex",gap:8}}>
                  <a href={r.url} target="_blank" rel="nofollow noopener" style={{ flex:1, display:"block",textAlign:"center",padding:"8px",borderRadius:8,fontSize:12,fontWeight:700,border:`1.5px solid ${i===0?T.green:T.border}`,background:i===0?T.green:"transparent",color:i===0?"white":T.dark, textDecoration:"none" }}>
                    Перейти на сайт →
                  </a>
                  <div style={{ flex:1 }}><AddBtn added={!!added[ID.realty(r.id)]} onAdd={() => handleAddRealty(r)}/></div>
                </div>
              </div>
            ))}
